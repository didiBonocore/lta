...
| P | `analyse:anonymise [--source= --out= --map=] [--emit-map] [--appendix] [--readme] [--harden] [--scan-repo] [--prune]` | The data-availability artefact: writes an anonymised, publishable copy of the raw export directory. Corpus lists pass through named (the study's input); every measurement-bearing file is aliased to stable `R01…` pseudonyms; the alias map — the join — is kept locally and withheld from the output unless `--emit-map`. A verification gate then re-reads the whole output and fails the run (deleting it) on any surviving repository name or segment, commit SHA, URL, or email, with every exemption listed in the run summary; files without a transformation rule are skipped, never copied. `--appendix` and `--readme` generate the Appendix A corpus table and the artefact README; `--harden`, `--scan-repo`, and `--prune` cover repository hygiene. |
